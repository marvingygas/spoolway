...
if drive base gone 200; then ok "the bottom of the chain runs and is archived"
...
# dispatch pass to cut one for real. `pipeline:` is required now, so this
# fills in the built-in `default` pipeline unless a caller already named
# its own — `spoolway stack` never resolves one itself, but a task file
# missing the key should still read like every other fixture's.
sib_task() {
  local id=$1; shift

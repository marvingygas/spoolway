...
#               it spends nothing.
smoke_suites=(flow)
pr_suites=(flow commands command-steps issue-tracking stacking stack checks-retry conflicts forge disaster lock trials routines jobs jobs-screen board-pause queue-unqueue restart overrides herdr-bind)
nightly_suites=(flow commands command-steps issue-tracking stacking stack checks-retry conflicts forge disaster lock trials routines jobs jobs-screen board-pause queue-unqueue restart overrides upgrade herdr-bind)
cloud_suites=(warmth)
live_suites=(live)

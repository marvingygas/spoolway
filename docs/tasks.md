...
| `usage_limit_hold` | dormant | The usage-limit detector that read a pane for its kind's phrase is gone, so nothing writes this now; a lane sitting on such a phrase is now an ordinary quiet pane, handled by `check_unreported` like any other. The field stays on the document — a park already on the file from before this shipped is left as-is. Cleared wherever `attempts` is, and by re-queueing the document |
| `parked_until` | dormant | Epoch seconds before which no lane is started for this task and no reminder is sent to a lane already running one. Nothing writes this any more — the quota gate and the usage-limit detector that used to set it are gone — but a park already on the file, from before this shipped or set by hand, is still honoured: `Dispatcher::parked` reads it from cold without taking a reading, and the board stops nudging the lane. Cleared only on a true exit from the hold, never by a deadline simply running out — an expired `parked_until` is left exactly as the file carries it, the pass answering `false` without touching the park so no reader sees a half-cleared hold; the same pass then resolves the whole park in one write, or launches a lane that drops the park fields. Also cleared by re-queueing the document |
| `quota_retries` | dormant | Consecutive rechecks that used to back a one-minute-to-one-hour doubling backoff; nothing runs this any more, the gate and detector that ran it being gone. The field stays on the document and is cleared together with the other park fields on a true exit from a hold. Cleared on stage changes, requeueing, successful admission, or a running lane leaving its hold. Repeated holds do not append duplicate status-log entries |
| `parked_window` | dormant | Which of the kind's two windows `parked_until` came from, `five_hour` or `seven_day`; it controls formatting of that recheck clock for the compatible `queue list --json` field. Nothing writes this any more, but a park already on the file still carries it, and `Dispatcher::parked` reads it to shape the report line. Empty for a mid-turn hold without an observed window. Cleared wherever `parked_until` is |
| `parked_at` | dormant | Epoch seconds when one continuous park began — fixed for the whole hold, unlike `parked_until` beside it, which is only the next recheck deadline. Nothing writes this any more, but the board and plain queue render `now - parked_at` as elapsed park age for a park already on the file, and `queue list --json` exposes the elapsed value separately as `parked_age`. A park without this field gets no invented age. Cleared on a true exit from the hold, wherever `parked_until` and `parked_window` are cleared: `set_stage` and `set_stage_unbanked`, `launch_landed` when a lane of it is seen running, the launch clear in `start_one` once the lane has actually started, and re-queueing the document |

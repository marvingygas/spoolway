...
| `usage_limit_hold` | the dispatcher | Whether the launch counted in `attempts` ended on its agent kind's own usage-limit message rather than dying outright — a spent quota takes the same doubling backoff whether or not anybody is watching, instead of escalating. Cleared wherever `attempts` is |

...
| `usage_limit_hold` | the dispatcher | Whether this task is held for its agent kind's own usage-limit message rather than for a dead launch. Informational only: the hold itself is `parked_until` now, not a doubling backoff, and nothing reads this to decide anything. It is here so a person — or a re-queue — can tell a quota hold apart from an ordinary park. Cleared wherever `attempts` is, and by re-queueing the document |
| `parked_until` | the dispatcher | Epoch seconds before which no lane is started for this task and no reminder is sent to a lane already running one. Written two ways: ahead of a launch, when the profile's kind is at or above its `quota_ceiling`, from the tripped window's own `resets_at`; and for a lane whose pane already carries its kind's usage-limit phrase, from the five-hour reset when a reading can be taken and from a doubling backoff when it cannot. The park lives on the file rather than in the dispatcher, so a dispatcher stopped for the whole wait honours it from cold without taking a reading. Cleared by `set_stage` and `set_stage_unbanked`, by the pass itself the moment the timestamp is in the past, and by re-queueing the document |
| `parked_window` | the dispatcher | Which of the kind's two windows `parked_until` came from, `five_hour` or `seven_day`. It only decides how the clock is drawn — a seven-day park keeps its dated shape on its last day instead of collapsing to a bare `HH:MM`. Empty for the mid-turn usage-limit hold, which is always the five-hour clock, and reads as `five_hour` when unset |

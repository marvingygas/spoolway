...
| `escalated` | the dispatcher | Whether `parked_from` names a step `escalate_clock` gave up on, rather than a person's own keypress or Escape. The one fact a resume cannot otherwise tell once a lane gets back here: a person's interrupt genuinely changed nothing, and the resumed lane is told so by its prompt; an escalation reminded the lane three times, tore its pane down and wrote a `## Status Log` line saying why. Written by `escalate_clock` alongside `parked_from`, and cleared wherever `parked_from` is |
...
| `trial` | the queue screen's `p` picker | The trial this task is one arm of, minted once per trial and stamped on every arm forked together. Absent on a task queued the ordinary way. What lets `spoolway eval --runs --trial <id>` find a trial's arms together in the ledger, since a trial forks a whole group, one arm per source task (`alpha-1`, `beta-1`, …) from different source documents that share nothing but this field — not even an id prefix — to group them by — see [Trials](planning.md#trials) |

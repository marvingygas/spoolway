...
| `pr` | flow, commands, stacking, stack, conflicts, forge, disaster, lock, trials, routines, restart | The last task of a chain, through the `suite` step in `.spoolway/pipelines/*.yml` |
...
|---|---|---|
| `flow` | A task's whole life: queued → implement → review → handover → archived | Real detached processes, a real worktree, a real pull request |
...
| `stack` | `spoolway stack` itself: the squash to one commit, a refused lease, the empty-diff refusal, and `[stack.summary]`'s modes — the body taken verbatim from the task file, a model turn's printed output as the whole body, and the refusals for a half-set table or a missing template | Real git, and a forge double the command really shells out to |

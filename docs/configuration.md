...
| `auto_commit` | `true` | Whether spoolway commits what a lane left uncommitted when its step settles, as `wip(<task>): <step>` — also the first thing `spoolway stack` does at `handover`, on the same rule. A lane that made its own commits is left alone and its leftovers reported as residue. `false` reports and commits nothing. Either way, a task on its way to the reserved `done` stage is held at `blocked` rather than archived when work is still in its worktree that spoolway could not record — `auto_commit` off with leftovers behind, or a git command that failed — so a worktree is never torn down over work that was never committed |
...
once an entry passes `retention_days`; `queue/`, `pending/`, `worktrees/`, `plans/` and
`overrides/` hold work in flight and are never swept, at any age.

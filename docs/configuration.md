...
The shipped Jira hooks are unchanged. Jira has no pull request lifecycle of its own, so
`jira.sh` and `jira.ps1` still transition the epic on the group's last `done`.

...
3. **Ask the three questions the cheaper passes could not.**
   - **Correct on the inputs nobody tried?** The empty one, the absent one, the one that
     arrives twice, the wrong type, the largest one allowed.
   - **Does it fit the system, or only the file?** A second concept where one already existed,
     a layer reached through, state held in two places that can now disagree.
...
4. **Check earlier findings were answered rather than worked around.** A criterion met by a
   ...
   added where a fix was asked for — each looks green from every angle except this one.
5. **Be specific, and be few.** Three findings that are each a real failure beat eleven that
...
6. **Pass a change that is correct and boring.** This is the last step that can stop a change,
...
When you cannot tell which list something belongs in, ask what happens if it ships. A defect
somebody will hit is a finding; a thing you would sigh at is a note.

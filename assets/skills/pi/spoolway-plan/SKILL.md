...
**The person is in the room now, and will not be later.** Every question is asked in chat
output, never written onto the page. **Do not write the page until every question is settled
out loud.**

**pi has no dialog tool, so a question is printed and the turn ends there.** Number the
questions, letter the options under each, and give every option its own consequence line.
Then stop and write nothing further — the person answers in their next prompt, by letter or
...
- **2–3 concrete options per question**, each a real route with its own consequence — "What do
  you think?" is a delay, not a question. **Ask in batches**, up to four per turn.
- **A choice between two shapes gets drawn** — print the two figures in the output, one under
...
1. **Understand the goal**, per above. When the goal names an issue — a ref, a link, "issue
   ...
   research subagents** as a printed question, and spawn them only on a yes.

2. **Settle everything, before a single line of the page is written.** List every open question
   and put them to the person as printed questions, batched, then end the turn.

3. **Ask whether the page is wanted.** Once the shape is settled, put it to them with
   a printed question — *Write the plan page?* — two answers: **Write it** (step 4), or **Cut
...
6. **Open it, say one line, and ask.** `wslview`, `xdg-open` or `open` — first that exists —
   ...
   **Then put the approval to them, printed, in the same turn** — never skipped,

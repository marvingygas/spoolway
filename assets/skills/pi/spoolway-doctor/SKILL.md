...
4. **Report, then ask.** Failures first, in the order they would stop a run; then the notes
   ...
   Then **print the questions and end the turn**: the mechanical fixes batched as one option
   and named individually in its description, each decision as its own question with its real
   options, and in every case an option that changes nothing. pi has no dialog tool, so the
   answers arrive as the person's next prompt. If nothing failed and no note

...
The `/spoolway-calibrate` skill reads your archived tasks and the spend ledger. It finds what
cost you loops and money, such as a prompt that keeps sending work back or a step on a model
bigger than it needs.

It walks each finding with you, with the numbers behind it. The ones you keep become tasks,
so the fix to your pipeline goes through the same pipeline as everything else.

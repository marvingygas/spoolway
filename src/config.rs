...
/// sessions should count as this project's spend — the sessions a person
/// runs in the base checkout to plan work and cut its tasks, which
/// `usage.jsonl` never sees today because nothing there is a lane. The
/// project root itself is never named here; it is always in the resolved
/// set — see [`Config::watch_roots`], which is also the only reader of this
/// list so far. Nothing here reads a transcript.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default, deny_unknown_fields)]

...
/// Save every task `validate_batch` handed back, and the same per-task report
/// [`finish_submit`] builds for its own batch — [`finish_trial`] no longer
/// builds this shape itself; see [`trial_report`] for what a trial reports
/// instead. The source documents under `.spoolway/routines/` are never
/// touched — a routine is meant to be queued again, not consumed by being
/// queued once.

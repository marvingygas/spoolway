...
3. **Say these five things, in this order, and then stop.** A heading with nothing under it
   gets three words rather than a paragraph.

       Tree:             assets/  |  .spoolway/  |  both — five words on which, and why
...
4. **The `Tree:` line is not a formality.** Almost everything here exists twice: the product
   under `assets/`, and this project's own live installation under `.spoolway/`. A change to a
   prompt, a pipeline, a task skeleton or a skill is supposed to land under `assets/`, so a
   diff that touched `.spoolway/` is the fastest way for a person to see that a task went wrong.
   That is why it goes first.
5. **Use numbers where you have them.** Your change's own diffstat gives you files changed and
   lines for free. A number is shorter than the sentence it replaces and much harder to shade.
6. **Leave that summary as your handoff**, one entry per line above. The summary is the whole of
   your output, and the task file is where the person deciding will be looking.
7. **Say what you are unsure of instead of smoothing it.** This exists so attention and money
   are not spent on a change that is not ready. One that reads well over a change that is not
   ready has done the opposite of its job.
8. **If the state does not add up** — the diff does not match what the handoffs claim, or there
   are criteria you have no way to evaluate — say so, and treat it as the block it is.

## Never

- Never edit, commit or push anything. Not a formatting fix, not a typo.
